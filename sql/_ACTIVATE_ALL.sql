...
--
-- ⚠ 이 파일은 계정 '골격'만 만든다(임의 비밀번호 → 로그인 불가).
--    실행 직후 sql/set_passwords.sql 을 각자 값으로 채워 한 번 더 실행해야 로그인이 된다.
--    저장소가 공개이므로 비밀번호를 파일에 적지 않는다.
--
...
-- ========== 다음 단계 ==========
-- sql/set_passwords.sql 을 열어 자리표시자를 실제 비밀번호로 바꾼 뒤 실행할 것.
-- 그 전에는 어떤 계정도 로그인할 수 없다(임의값으로 생성됨).
