...
신뢰성 검증 기능 통합 회귀. 화면 HTML 을 실제 `<script>` 순서 그대로 jsdom 에 띄우고 케이스 상태를 주입해 관측한다. 102개 단언.

| 절 | 확인 내용 |
|---|---|
| 1 | 검증 규칙 단위 — CAS 체크디짓 · 농도 상충 · 도로법 운행제한 · 운송기준(2인 · 예외 · 비대상) · 법령 대조 · 지정수량 |
| 2 | 구현·검증 페이지 — 범위 17행 · 전환 기준 13행 · 법령 9행 · 절차 단계 |
...
| 11 | 원본 정답지 채점 — 시연 데이터 30/30 · 쪽수 · 잘못된 값 감점 |
| 12 | 문서 특성(언어 · 품질) 전환 · 보관 온도 도출 · 물품(Article) · MSDS '문서 특성' 표시 |

jsdom 에는 `matchMedia` 가 없어 하네스가 최소 스텁을 넣는다(브라우저에는 있음).

## msds-eval.js — MSDS 정답지 채점

`eval/gold/*.json` 은 실제 MSDS 원본 PDF에서 옮긴 정답지다(원문 표기 · 쪽수 · 총 쪽수 · 원문 오기).

```bash
node msds-eval.js                 # 시연 데이터(js/data_dg.js) 채점 — 원본과 어긋나면 실패 종료
node msds-eval.js --pred <폴더>    # 분석 엔진 결과(<샘플ID>.json) 채점 — 점수만 보고
node msds-eval.js --json          # 결과 JSON
```
